...
const RegistrarsPage = () => {
  ...
  const fetchRegistrars = async () => {
    try {
      setLoading(true);
      const response = await getRegistrars({ status: statusFilter, limit: 100, search: search.trim() || undefined });
      if (response.success) {
        setRegistrars(response.data?.items || []);
        setError('');
      } else {
        setError(response.error?.message || 'Failed to load registrars.');
      }
    } catch (err) {
      setError(getErrorMessage(err, 'Failed to load registrars.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRegistrars = useMemo(() => {
    ...
    return registrars.filter(
      (registrar) =>
        String(registrar.full_name || '').toLowerCase().includes(q) ||
        String(registrar.email || '').toLowerCase().includes(q) ||
        String(registrar.phone || '').toLowerCase().includes(q)
    );
  }, [registrars, search]);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      setSaving(true);
      const response = await createRegistrar(createForm);
      if (!response.success) {
        setError(response.error?.message || 'Failed to create registrar.');
        return;
      }
      setCreateOpen(false);
      ...
      setError(getErrorMessage(err, 'Failed to create registrar.'));
    } finally {
      setSaving(false);
    }
  };

  const openEdit = (registrar) => {
    setEditTarget(registrar);
    const names = String(registrar.full_name || '').trim().split(' ');
    setEditForm({
      first_name: names[0] || '',
      last_name: names.slice(1).join(' ') || '',
      email: registrar.email || '',
      phone: registrar.phone || '',
      gender: registrar.gender || 'M'
    });
  };
  ...
  const handleEdit = async (e) => {
    ...
    try {
      setSaving(true);
      const response = await updateRegistrar(editTarget.id, editForm);
      if (!response.success) {
        setError(response.error?.message || 'Failed to update registrar.');
        return;
      }
      setEditTarget(null);
      ...
      setError(getErrorMessage(err, 'Failed to update registrar.'));
    } finally {
      setSaving(false);
    }
  };

  const toggleStatus = async (registrar) => {
    try {
      setSaving(true);
      const response =
        registrar.status === 'active'
          ? await deactivateRegistrar(registrar.id)
          : await activateRegistrar(registrar.id);
      if (!response.success) {
        setError(response.error?.message || 'Failed to update registrar status.');
        return;
      }
      await fetchRegistrars();
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500">Manage registrar accounts for your school.</p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredRegistrars.map((registrar) => (
                <tr key={registrar.id} className="border-t border-gray-100">
                  <td className="px-4 py-3">{registrar.full_name}</td>
                  <td className="px-4 py-3">
                    <div className="text-xs text-gray-600">
                      <div>{registrar.email || '-'}</div>
                      <div>{registrar.phone || '-'}</div>
                    </div>
                  </td>
                  <td className="px-4 py-3">{registrar.gender || '-'}</td>
                  <td className="px-4 py-3">
                    <span
        ...
        <div className="fixed inset-0 bg-black/30 z-40 flex items-center justify-center p-4">
          <form onSubmit={handleCreate} className="w-full max-w-2xl bg-white rounded-xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">Create Registrar</h3>
      ...
      {editTarget ? (
        <div className="fixed inset-0 bg-black/30 z-40 flex items-center justify-center p-4">
          <form onSubmit={handleEdit} className="w-full max-w-2xl bg-white rounded-xl p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold">Edit Registrar</h3>
