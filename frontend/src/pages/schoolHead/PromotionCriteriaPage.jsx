// Promotion Criteria Management Page (School Head)
// List, create, edit, delete promotion criteria for the school
...
const CriteriaModal = ({ isOpen, mode, criteria, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // School head API expects: name, passing_average, passing_per_subject, max_failing_subjects, is_active
      const payload = {
        name: formData.name,
        passing_average: formData.passing_average,
        passing_per_subject: formData.passing_per_subject,
        max_failing_subjects: formData.max_failing_subjects,
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="is_active" className="text-sm text-gray-700">
              Active (use for promotion decisions)
            </label>
          </div>
...
const PromotionCriteriaPage = () => {
  ...
  const handleSave = async (payload) => {
    if (modalMode === 'create') {
      await createPromotionCriteria(payload);
    } else {
      await updatePromotionCriteria(selectedCriteria.id, payload);
    }
    fetchCriteria();
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Manage promotion rules for your school
          </p>
        </div>
        <button
