// Setup MAMP Local Database
// Run this with: node setup-database.js
// Make sure MAMP is running with MySQL on port 3306
...
async function setupDatabase() {
  ...
  try {
    console.log('🔧 Setting up MAMP local database...\n');
    ...
    // Create database
    const dbName = process.env.DB_NAME || '2schoolportal';
    ...
    const dropOrder = [
      'transcripts', 'rosters', 'student_semester_results', 'grade_submissions',
      'marks', 'assessment_weights', 'weight_templates', 'assessment_types',
      'teaching_assignments', 'student_parents', 'students', 'subjects',
      'classes', 'grades', 'semesters', 'academic_years', 'users', 'schools',
      'promotion_criteria'
    ];
    ...
    console.log('Next step: run "node seed-database.js" to add test data');

  } catch (error) {
    console.error('\n❌ Setup failed:', error.message);
    console.error('\nMake sure:');
    console.error('1. MAMP is running');
    console.error('2. MySQL is started on port 3306');
    console.error('3. Root password is "root" (MAMP default)');
    process.exit(1);
  } finally {
    if (connection) {
